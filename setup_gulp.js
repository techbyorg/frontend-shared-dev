...
export default (function({config, Lang, paths}) {
  ...
  gulp.task('dist:concat', function(done) {
    ...
    _map(matches, function(match) {
      ...
      return bundle = bundle.replace(match, `'${process.env[key]}'`);
    });
  ...
  gulp.task('watch', () => gulp.watch(paths.coffee, ['dev:server']));

  gulp.task('watch:dev:server', gulp.series('dev:server', () => gulp.watch(paths.coffee, ['dev:server']))
  );
